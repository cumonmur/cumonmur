...
low_hsv_yellow = (21, 153, 165)
max_hsv_yellow = (27, 255, 255)
...
def keep_yaw(yaw_to_set, speed, k = 1):
    ...
    auv.set_motor_power(2, -res - speed)
    return e
...
def search_buoys_in_order(img):  # Поиск Буёв по порядку (Зелёный, Жёлтый, Красный) и Сразу совместил, что он их косаится (я не русский)
    ...
    while i <= 0:
        if color == "green":
            remembered_color.append("green")
        elif color == "yellow":
            remembered_color.append("yellow")
        elif color == "red":
            remembered_color.append("red")
...
while True:
   ...
   while time.time() - time_flag < 1:
       keep_yaw(yaw, speed=20)
   print("d1")
   time_flag = time.time()
   while time.time() - time_flag < TIMER_STAB_GATE:
       e = gate()
       if e > 10:
           time_flag = time.time()
   yaw = auv.get_yaw()
   ...
   time_flag = time.time()
   print("go gate")
   while time.time() - time_flag < TIMER_GO_GATE:
       keep_yaw(yaw, speed= 30)

   print("go_bue")
   time_flag = time.time()
   while time.time() - time_flag < TIMER_GO_BUE:
       keep_yaw(yaw, speed=25)
   ...
   print("turn")
   time_flag = time.time()
   while time.time() - time_flag < TIMER_TURN_BUE:
       auv.set_motor_power(1, -50)
       auv.set_motor_power(2, 70)
   ...
   time_flag = time.time()
   while time.time() - time_flag < TIMER_GO_GATE * 2:
       keep_yaw(yaw, speed=25)
